...
function nextExercise() {
  ...
  if (step >= EXERCISES.length) {
    ...
    ui.exerciseImg.style.display = 'none';
...
function setExercise(i) {
  ...
  ui.exerciseImg.src         = EXERCISES[i].img;
  ui.exerciseImg.style.display = 'block';
}
...
function buildUI() {
  ...
  Object.assign(exerciseImg.style, {
    display:      'none',
    width:        '500px',
    height:       'auto',
    objectFit:    'contain',
    ...
    padding:      '8px',
    border:       '1px solid rgba(181,211,255,0.20)',
  });
  exerciseImg.alt = 'ejercicio';
  ...
  panel.append(label, instruction, exerciseImg, status, divider, row);
  document.body.appendChild(panel);

  ui = { label, instruction, exerciseImg, status, ring, timerText, webcamWrap };
}
